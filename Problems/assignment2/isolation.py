...
class Board:
    ...
    def set_state(self, board_state, p1_turn=True):
        ...
        # keep __last_forcefield_pos__ in sync with the state we just set
        self.__last_forcefield_pos__ = [
            (r, c)
            for r in range(self.height)
            for c in range(self.width)
            if board_state[r][c] == Board.FORCEFIELD
        ]

        # Locate queens
        q1 = self.__queen_symbols__[self.__queen_1__]
        q2 = self.__queen_symbols__[self.__queen_2__]

        last_move_q1 = [(r, row.index(q1)) for r, row in enumerate(board_state) if q1 in row]
        last_move_q2 = [(r, row.index(q2)) for r, row in enumerate(board_state) if q2 in row]

        self.__last_queen_move__[self.__queen_1__] = last_move_q1[0] if last_move_q1 else Board.NOT_MOVED
        self.__last_queen_move__[self.__queen_2__] = last_move_q2[0] if last_move_q2 else Board.NOT_MOVED
    ...
    # function to edit to introduce any variant - edited for trail isolation variant 9/1/2025
    def __apply_move__(self, queen_move):
        ...
        ######Changed the following lines to introduce "trail" variant######
    ...
    def __create_forcefield__(self, current_position):
        """
        For the *trail* variant: mark the intermediate squares on the line
        from the mover's previous position to current_position as a
        one-turn FORCEFIELD (impassable for the opponent's next move).
        """
        curr_row, curr_col = current_position
        prev_row, prev_col = self.__last_queen_move__[self.__active_players_queen__]

        # If this queen hasn’t moved yet, don’t paint a trail (no phantom trails at start of game)
        if (prev_row, prev_col) == Board.NOT_MOVED:
            return

        # step direction (queen line: horizontal, vertical, or diagonal)
        dr = curr_row - prev_row
        dc = curr_col - prev_col
        step_r = 0 if dr == 0 else (1 if dr > 0 else -1)
        step_c = 0 if dc == 0 else (1 if dc > 0 else -1)

        # must be straight or diagonal; otherwise no trail to paint
        if not (step_r == 0 or step_c == 0 or abs(dr) == abs(dc)):
            return

        # walk from the square *after* prev to the square *before* curr
        r, c = prev_row + step_r, prev_col + step_c
        while (r, c) != (curr_row, curr_col):
            # place only on blank, in-bounds cells (never endpoints)
            if self.move_is_in_board(r, c) and self.__board_state__[r][c] == Board.BLANK:
                self.__last_forcefield_pos__.append((r, c))
                self.__board_state__[r][c] = Board.FORCEFIELD
            r += step_r
            c += step_c
    ...
    def get_active_players_queen(self):
        """
        See which queen is active. Used mostly in play_isolation for display purposes.
        Parameters:
            None
        Returns:
    ...
    def get_inactive_moves(self):
        ...
        # Save/clear current one-turn forcefield
        saved_ff = list(self.__last_forcefield_pos__)
        self.__clear_forcefield__()

        # Compute moves for the inactive player (they should NOT be blocked by their own trail)
        moves = self.__get_moves__(q_move)

        # Restore exactly the same temp trail cells
        for (r, c) in saved_ff:
            if self.__board_state__[r][c] == Board.BLANK:
                self.__board_state__[r][c] = Board.FORCEFIELD
        self.__last_forcefield_pos__ = saved_ff
    ...
    def play_isolation(self, time_limit=10000, print_moves=False):
        ...
        # Take first move for each player once randomly, to avoid playing the same game
        for _ in range(2):
            moves = self.get_active_moves()
            
            # Filter out edge moves
            non_edge_moves = [(r, c) for r, c in moves 
                              if 0 < r < self.height-1 and 0 < c < self.width-1]
            
            # If no non-edge moves are available, fall back to all available moves
            available_moves = non_edge_moves if non_edge_moves else moves
            
            curr_move = random.choice(available_moves)
            _, _, winner = self.forecast_move(curr_move)
            if winner:
                while winner:
                    curr_move = random.choice(available_moves)
                    _, _, winner = self.forecast_move(curr_move)
